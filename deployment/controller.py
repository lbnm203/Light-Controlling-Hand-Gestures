...
class ModbusMaster():
    def __init__(self) -> None:
        ...
        if which_os == "Linux":
            name_ports = list(filter(lambda name: "USB" in name, map(
                lambda port: port.name, port_list)))
        ...
        print(self.ser.baudrate, self.ser.stopbits,
              self.ser.parity, self.ser.bytesize)
